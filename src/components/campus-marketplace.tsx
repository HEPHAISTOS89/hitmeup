        ...
        <section className="workspace">
          <aside className="discovery-panel" aria-label="Discovery filters">
            ...
            <section className="smart-discovery" aria-labelledby="smart-discovery-title">
              <div className="smart-discovery-heading">
                <div className="smart-discovery-label"><span className="gemini-mark"><Sparkles size={14} /></span><strong id="smart-discovery-title">Find with Gemini</strong></div>
                {dataMode === "live" ? <label className="gemini-consent gemini-consent-chip"><input type="checkbox" checked={smartConsent} onChange={(event) => setSmartConsent(event.target.checked)} /><span>Use Gemini</span></label> : <span className="smart-preview-label">Preview</span>}
              </div>
              <div className="smart-discovery-compose"><input value={smartPrompt} maxLength={500} onChange={(event) => { setSmartPrompt(event.target.value); setSmartResult(null); }} onKeyDown={(event) => { if (event.key === "Enter") void runSmartDiscovery(); }} placeholder="What do you need nearby?" aria-label="Describe what you need for Gemini" /><button type="button" disabled={smartBusy || !smartPrompt.trim() || (dataMode === "live" && !smartConsent)} onClick={() => void runSmartDiscovery()}>{smartBusy ? <LoaderCircle className="spin" size={15} /> : <Navigation size={15} />}<span>{smartBusy ? "Thinking" : "Find"}</span></button></div>
