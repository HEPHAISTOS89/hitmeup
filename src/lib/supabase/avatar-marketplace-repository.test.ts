...
describe("avatar marketplace repository", () => {
  it("maps the full server catalog contract", async () => {
    const rpc = vi.fn(async () => ({ data: [{
      sku: "avatar.accessory.beanie", label: "Night beanie", category: "accessory", value_key: "beanie",
    ...
    await expect(listAvatarMarketplace({ rpc } as never)).resolves.toEqual([{
      sku: "avatar.accessory.beanie", label: "Night beanie", category: "accessory", value: "beanie",
  ...
  it("reads the subject reward summary and sends no client-authored point amount", async () => {
    const rpc = vi.fn(async (name: string) => ({ data: name === "get_my_reward_summary" ? [{ balance: 50, lifetime_earned: 100, lifetime_spent: 50, unlocked_skus: ["avatar.accessory.beanie"] }] : true, error: null }));
    await expect(getRewardSummary({ rpc } as never)).resolves.toEqual({ balance: 50, lifetimeEarned: 100, lifetimeSpent: 50, unlockedSkus: ["avatar.accessory.beanie"] });
    await unlockRewardCosmetic({ rpc } as never, "avatar.accessory.beanie");
    expect(rpc).toHaveBeenLastCalledWith("unlock_my_avatar_reward", { target_sku: "avatar.accessory.beanie" });
  });
  ...
  it("binds a paid claim to the server reservation and verified amount", async () => {
    ...
    await expect(reserveCosmeticQuote({ rpc } as never, "auth0|student", "avatar-premium-collection", 50_000_000, "00000000-0000-4000-8000-000000000002")).resolves.toBe(quoteId);
