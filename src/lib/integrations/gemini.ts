...
const INTERNATIONAL_PHONE_PATTERN = /(?<![\w\d])(?:\+|00)?\d[\d\s().-]{6,}\d(?![\w\d])/g;
...
const STREET_ADDRESS_PATTERN = /\b\d{1,6}\s+(?:[A-Z0-9][\w.'-]*\s+){0,6}(?:street|st|avenue|ave|road|rd|drive|dr|lane|ln|boulevard|blvd|court|ct|circle|cir|highway|hwy|parkway|pkwy|way)\b(?:\s*(?:apt|unit|suite|room|rm)\s*#?[A-Z0-9-]+)?/gi;
const ROOM_PATTERN = /\b(?:room|rm|apartment|apt|suite|dorm|building|bldg)\s*[#:-]?\s*[A-Z0-9-]{1,16}\b/gi;
const NAMED_PERSON_PATTERN = /\b(?:[Mm]y name is|[Ii] am|[Ii]'m|[Cc]ontact|[Aa]sk for|[Mm]eet with|[Mm]eeting with)\s+[A-Z][A-Za-z'-]+(?:\s+[A-Z][A-Za-z'-]+){0,2}\b/g;
const CHAT_LINE_PATTERN = /\b(?:you|me|them|[A-Z][A-Za-z'-]{1,30}(?:\s+[A-Z][A-Za-z'-]{1,30})?):\s+[^.!?]{4,}(?:[.!?]|$)/g;

function cleanText(value: unknown, max = 2_000) {
  return typeof value === "string" ? value.replace(/[\u0000-\u001f\u007f]/g, " ").trim().slice(0, max) : "";
}

export function privacySafeText(value: unknown, max: number) {
  return cleanText(value, max)
    .replace(EMAIL_PATTERN, "[email omitted]")
    .replace(PHONE_PATTERN, "[phone omitted]")
    .replace(INTERNATIONAL_PHONE_PATTERN, "[phone omitted]")
    .replace(URL_PATTERN, "[link omitted]")
    .replace(COORDINATE_PATTERN, "[location omitted]")
    .replace(STREET_ADDRESS_PATTERN, "[address omitted]")
    .replace(ROOM_PATTERN, "[room omitted]")
    .replace(NAMED_PERSON_PATTERN, "[name omitted]")
    .replace(CHAT_LINE_PATTERN, "[private content omitted]")
    .replace(PRIVATE_CONTENT_PATTERN, "[private content omitted]");
}
