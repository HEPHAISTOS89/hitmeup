...
const ALLOWED_CATEGORIES = new Set(["Tutoring", "Tech help", "Ride", "Creative", "Moving", "Other"]);
...
export function deterministicSuggestion(draft: ServiceDraft): GeminiSuggestion {
  ...
  return {
    ...
    riskFlags: [],
    searchKeywords: [...new Set([category.toLowerCase(), ...tags])].slice(0, 8),
...
function validateSuggestion(value: unknown, fallback: GeminiSuggestion): GeminiSuggestion {
  ...
  const searchKeywords = Array.isArray(candidate.searchKeywords)
    ...
    : [];
  if (!ALLOWED_CATEGORIES.has(category) || !suggestedTitle || tags.length === 0 || searchKeywords.length === 0) return fallback;
  return { category, suggestedTitle, tags, riskFlags, searchKeywords, source: "gemini" };
}
...
export async function suggestService(draft: ServiceDraft): Promise<GeminiSuggestion> {
  ...
  const parts: Array<Record<string, unknown>> = [{ text: `Classify this student service. Return JSON only with category, tags, suggestedTitle, riskFlags, searchKeywords. Title: ${valid.title}\nDescription: ${valid.description}` }];
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseSchema: {
            ...
            required: ["category", "tags", "suggestedTitle", "riskFlags", "searchKeywords"],
          },
        },
      }),
    });
