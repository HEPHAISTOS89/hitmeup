...
describe.runIf(RUN_DEVNET)("real Solana Devnet cosmetic payment", () => {
  it("accepts the exact transfer and rejects a wrong amount and wrong payer", async () => {
    ...
    const exactSignature = await transfer(10_000_000);
    await expect(verifyCosmeticPayment(exactSignature, "profile-frame", payer.publicKey.toBase58())).resolves.toMatchObject({
      verified: true,
      network: "devnet",
    ...
    await expect(verifyCosmeticPayment(exactSignature, "profile-frame", Keypair.generate().publicKey.toBase58()))
      .rejects.toThrow("does not pay the requested cosmetic");

    const wrongAmountSignature = await transfer(9_999_999);
    await expect(verifyCosmeticPayment(wrongAmountSignature, "profile-frame", payer.publicKey.toBase58()))
      .rejects.toThrow("does not pay the requested cosmetic");
